...
// https://github.com/surmon-china/mongodb-data-api/pull/3/files
type Projection = FindOptions['projection']

// https://github.com/surmon-china/mongodb-data-api/pull/4/files
type NoInfer<A = any> = [A][A extends any ? 0 : never]
...
export class MongoDBDataAPI<InnerDoc = Document> {
  ...
  public $$action<Result = unknown>(
    ...
    if (!mergedParams.dataSource || !mergedParams.database || !mergedParams.collection) {
      return Promise.reject(new Error('Invalid params: dataSource, database, collection'))
    }
